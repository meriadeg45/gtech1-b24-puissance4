...
int replay_game() {               //permet au joueur de relancer le jeu, ou de le stopper
  int flag_replay_game = 0;
  if(isThereAWinner=true) {
    printf("Voulez vous rejouer ? entrez 1 pour OUI, 2 pour NON\n");
    int replay_choice = scanf("%d", &flag_replay_game);
    if(replay_choice < 1) {
      ...
      if(flag_replay_game == 1) {
        main();
      }
      else if(flag_replay_game == 2) {
        printf("Merci, à bientot !\n");
      }
      else {
        printf("ce choix n'est pas disponible.\n");
        flushstdin();
